"""Celery tasks: run a full HYDRA scan for a single target on a worker."""
...
from hydra.distributed.celery_app import app


@app.task(name="hydra.scan_target")
def scan_target(config_json: dict[str, Any]) -> dict[str, Any]:
    """Run recon -> scan -> exploit -> report for one target. Returns a summary."""
    from hydra.core.config import ScanConfig, get_settings
