...
soft-404. Before scanning, HYDRA sends a handful of deliberately nonsensical
...
def _probe_paths(token: str) -> list[str]:
    ...
    return [
        f"hydra-not-found-{token}",
        f"hydra-not-found-{token}/{secrets.token_hex(3)}",
        f"hydra-not-found-{token}.html",
    ]
