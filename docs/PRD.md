...
| Verified snapshot | 58 scanners · 17 confirmation modules · 18 recon modules · 1019 passing tests · `ruff` clean |
...
- **1019 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 13 |
| Automated tests | **1019** (ruff clean) |
...
tests/         unit + integration (1019 tests)
