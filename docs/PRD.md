...
| Verified snapshot | 56 scanners · 17 confirmation modules · 16 recon modules · 875 passing tests · `ruff` clean |
...
- **875 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 13 |
| Automated tests | **875** (ruff clean) |
...
tests/         unit + integration (875 tests)
