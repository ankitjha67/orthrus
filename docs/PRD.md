...
| Verified snapshot | 58 scanners · 17 confirmation modules · 16 recon modules · 936 passing tests · `ruff` clean |
...
- **936 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 13 |
| Automated tests | **936** (ruff clean) |
...
orthrus/
  ...
  main.py      Click CLI (23 commands)
docs/          README, PROOF.md, this PRD, screenshot
tests/         unit + integration (936 tests)
