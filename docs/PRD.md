...
| Verified snapshot | 55 scanners · 17 confirmation modules · 16 recon modules · 834 passing tests · `ruff` clean |
...
- **834 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 13 |
| Automated tests | **834** (ruff clean) |
...
tests/         unit + integration (834 tests)
