...
| Verified snapshot | 59 scanners · 28 confirmation modules · 18 recon modules · 1337 passing tests · `ruff` clean |
...
- **1337 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 30 |
| Automated tests | **1337** (ruff clean) |
...
tests/         unit + integration (1337 tests)
