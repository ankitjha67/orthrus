...
| Verified snapshot | 70 scanners · 28 confirmation modules · 18 recon modules · 1534 passing tests · `ruff` clean |
...
## 7. Vulnerability scanning subsystem (70 scanners)
...
| `sri` | sri | CWE-353 | PASSIVE; third-party `<script>`/`<link>` (different registrable domain) served over https without an `integrity=` hash → CDN-compromise / Magecart exposure; first-party assets and `http:` refs are left to the mixed-content scanner |
...
| `email_auth` | email-auth | CWE-290/16 | PASSIVE; passive DNS - parses SPF (missing / `+all` / `?all` / multi-record permerror) and DMARC (missing / `p=none` monitor-only / `sp=none` / `pct<100`, walked up to the organizational policy) → spoofable domain, the phishing / BEC enabler; best-effort DKIM common-selector probe |
...
- **1534 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 30 |
| Automated tests | **1534** (ruff clean) |
...
>
> **✅ Build-out Wave 4 shipped** (56→58 scanners, +20 tests, pure detectors - no new core infra):
> - **LDAP injection** - filter-breaking payloads → directory-server error signatures (JNDI/OpenLDAP/AD/PHP) - `ldap`, CWE-90.
...
orthrus/
  ...
  scanners/    70 scanners + base + registry + _injection + _evasion
...
tests/         unit + integration (1534 tests)
