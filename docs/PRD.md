...
| Verified snapshot | 60 scanners · 28 confirmation modules · 18 recon modules · 1352 passing tests · `ruff` clean |
...
TARGET →│  RECON   │ → │   SCAN   │ → │ EXPLOIT / CONFIRM  │ → │  REPORT  │→ artifacts
        │ 18 mods  │   │ 60 scan  │   │ 28 confirmers      │   │ 6 fmts   │
        └──────────┘   └──────────┘   └────────────────────┘   └──────────┘
             │              │                   │                    │
...
## 7. Vulnerability scanning subsystem (60 scanners)
...
- **1352 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 30 |
| Automated tests | **1352** (ruff clean) |
...
orthrus/
  ...
  scanners/    60 scanners + base + registry + _injection + _evasion
...
tests/         unit + integration (1352 tests)
