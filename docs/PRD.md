...
| Verified snapshot | 56 scanners · 17 confirmation modules · 16 recon modules · 870 passing tests · `ruff` clean |
...
## 7. Vulnerability scanning subsystem (56 scanners)
...
- **870 tests**, `ruff` clean (E,F,I,UP,B,ASYNC). Pure detector unit tests + duck-typed fakes + real-socket / real-process / real-browser integration checks (raw-socket desync, live JWKS forge, Chromium DOM taint, real gRPC reflection, OOB collaborator).
...
| CLI commands | 13 |
| Automated tests | **870** (ruff clean) |
...
orthrus/
  ...
  scanners/    56 scanners + base + registry + _injection + _evasion
...
tests/         unit + integration (870 tests)
