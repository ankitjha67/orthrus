...
ORTHRUS crawls a target, fingerprints its stack, runs 49 vulnerability scanners,
...
737-test / lint-clean quality gates.
...
| Access / logic | IDOR, **multi-identity authorization matrix (BOLA/BFLA, Autorize-style `--identities`)**, CSRF, open redirect, race conditions, business-logic (parameter tampering / HPP), host-header injection (password-reset poisoning) |
...
| Config / transport | Security headers, **CSP weakness analysis**, CORS, TLS analysis, exposed files, **directory-listing / autoindex**, cache poisoning, web cache deception, framework debug-exposure, unrestricted file upload, subdomain takeover, **HTTP misconfig (TRACE/XST, dangerous methods)** |
...
orthrus/
  ...
  scanners/    49 scanners + base interface + registry
