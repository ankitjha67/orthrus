...
registries. Discovers both the built-in ``orthrus/plugins`` package and an
optional external directory (``ORTHRUS_PLUGINS_DIR``).
"""
...
def load_plugins(extra_dir: str | None = None) -> list[str]:
    ...
    import orthrus.plugins as pkg

    for module in pkgutil.iter_modules(pkg.__path__):
        if module.name.startswith("_"):
            continue
        try:
            importlib.import_module(f"orthrus.plugins.{module.name}")
    ...
    if extra_dir and os.path.isdir(extra_dir):
        for filename in sorted(os.listdir(extra_dir)):
            ...
            try:
                spec = importlib.util.spec_from_file_location(f"orthrus_plugin_{name}", path)
